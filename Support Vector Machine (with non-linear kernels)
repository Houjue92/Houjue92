...
def polynomial_kernel(X1, X2,d=2):
    ...
    #########################################
    ## INSERT YOUR CODE HERE
    I = np.ones([X1.shape[0], X2.shape[0]])
    K = np.power(X1 * np.transpose(X2) + I, d)
    #########################################
    return K 
...
def predict(K, a, y, b):
    ...
    #########################################
    ## INSERT YOUR CODE HERE
    y_test = np.sum(np.multiply(np.transpose(np.multiply(a, y)), K), axis = 1) + b
    y_test[y_test > 0] = 1
    y_test[y_test <= 0] = -1
    #########################################
    return y_test
...
def compute_E(Ki,a,y,b,i):
    ...
    #########################################
    ## INSERT YOUR CODE HERE
    fx = np.sum(np.multiply(np.transpose(np.multiply(a, y)), Ki), axis = 1) + b
    E = float(fx[0][0]-y[i][0])
    #########################################
    return E
    ...
    for _ in xrange(n_epoch):
        for i in xrange(n):
            for j in xrange(n):
                ...
                # compute eta 
                eta = compute_eta(float(K[i,i]),float(K[j,j]),float(K[i,j]))
                ...
                a[i] = ai_new
                a[j] = aj_new
                b = update_b(b,ai_new,aj_new,ai,aj,yi,yj,Ei,Ej,float(K[i,i]),float(K[j,j]),float(K[i,j]),C)
