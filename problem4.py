...
class LSTM(sr):
    ...
    def gates(self, x, H):
        ...
        ## INSERT YOUR CODE HERE
        x_H = th.cat((x,H),1)
        f = nn.functional.sigmoid(x_H.mm(self.W_f) + self.b_f)
        i = nn.functional.sigmoid((x_H.mm(self.W_i) + self.b_i))
        o = nn.functional.sigmoid((x_H.mm(self.W_o) + self.b_o))
        C_c = tanh(x_H.mm(self.W_c) + self.b_c)
        ...
        while True:
            ...
            for x,y in loader:
                ...
                for i in range(t):
                    # forward pass
                    xt = x[:, i]
                    z, H_new, C_new = self.forward(xt, H, C)
                    H = H_new
                    C = C_new
                    # compute loss 
                    L += self.loss_fn(z, y[:, i])
