...
def compute_a2(z2):
    ...
    for i in range(c):
        if z2[i] > 100:
            ...
            e2[i][0] = math.exp(z2[i])
    a2 = np.mat(e2 / np.sum(e2))
    #########################################
    return a2 
...
def compute_dz2_dW2(a1,c):
    ...
    ## INSERT YOUR CODE HERE
    h = len(a1)
    dz2_dW2 = np.zeros([c,h])
    for i in range(c):
        for j in range(h):
            dz2_dW2[i][j] = a1[j]
    dz2_dW2 = np.mat(dz2_dW2)
    #########################################
    return dz2_dW2
...
def compute_dz1_dW1(x,h):
    ...
    ## INSERT YOUR CODE HERE
    p = len(x)
    dz1_dW1 = np.zeros([h,p])
    for i in range(h):
        for j in range(p):
            dz1_dW1[i][j] = x[j]
    dz1_dW1 = np.mat(dz1_dW1)
    #########################################
    return dz1_dW1
...
def compute_dL_da1(dL_dz2,dz2_da1):
    ...
    #########################################
    ## INSERT YOUR CODE HERE
    dL_da1 = np.transpose(dz2_da1) * dL_dz2
    #########################################
    return dL_da1
