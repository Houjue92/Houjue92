...
class QNet(object):
    ...
    def __init__(self, n=4, d=16, e=0.1):
        ...
        self.n = n
        self.e = e
        self.W = Variable(th.FloatTensor(n, d).zero_(), requires_grad=True)
    ...
    def forward(self, s):
        ...
        if np.random.random()<self.e:
            a = np.random.random_integers(0,self.n-1)
        else:
            Q = self.compute_Q(s)
            value,index = th.max(Q,0)
            a = int(index)
    ...
    def compute_L(self,s,a,r,s_new, gamma=.95):
        ...
        # compute target Q
        target = r + gamma*(max(self.compute_Q(s_new).data))
        # get current Q
        current = self.compute_Q(s)[a]
        # compute loss
        L = (target - current)**2
        #########################################
        return L 
